...
void ssd1306_DrawLine(uint8_t X,uint8_t Y,uint16_t ang,uint16_t len,uint16_t color)
{
	//функция рисования линии.
	// х,у - начальные координаты
	// ang - угол поворота.  относительно трех часов
	// len - длина линии
	// цвет
	...
	case 0:
		...
		for (i = 0; i<dx; i++)
			{
			y = k*i;
			ssd1306_DrawPixel(X + i, round(Y + y), color);
			}
		break;
	case 1:
		...
		for (i = 0; i<dx; i++)
			{
			y = k*i;
			ssd1306_DrawPixel( round(X + y), Y + i, color);
			}
		break;
	case 2:
